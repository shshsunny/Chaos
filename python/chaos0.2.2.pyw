...
c8 = [207, 224, 234]
BACKGROUND = [6, 3, 24]
filler1 = [BACKGROUND[0] + c1[0], BACKGROUND[1] + c1[1], BACKGROUND[2] + c1[2]]
filler2 = [filler1[0] // 2, filler1[1] // 2, filler1[2] // 2]
filler1[0] //= 16
filler1[1] //= 16
filler1[2] //= 16
filler1 = BACKGROUND[:]
...
def shot():
    ...
    for i in range(random.randint(30, 60)):
        x = random.uniform(-1/1.6, 1/1.6)
        y = random.uniform(-1/1.6, 1/1.6)
        percent = random.uniform(0.1, 1)
        star1_shots.append([[x, y], [c8[0]*percent, c8[1]*percent, c8[2]*percent]])

def draw_shots(center, radius):
    for _s in star1_shots:
        pygame.draw.circle(s, (int(_s[1][0]), int(_s[1][1]), int(_s[1][2])), [int(center[0]+radius*_s[0][0]), int(center[1]+radius*_s[0][1])], int(radius / random.randint(25, 55)+0.5))
        _t = random.uniform(-2, 2)
        _s[1][0] += _t
        _s[1][1] += _t
        _s[1][2] += _t
        _s[1][0] = min(_s[1][0], 255)
        _s[1][1] = min(_s[1][0], 255)
        _s[1][2] = min(_s[1][0], 255)
        _s[1][0] = max(_s[1][0], 0)
        _s[1][1] = max(_s[1][1], 0)
        _s[1][2] = max(_s[1][2], 0)

        _s[0][0] *= random.uniform(0.99, 1.01)
        _s[0][1] *= random.uniform(0.99, 1.01)
        while _s[0][0] > 1/1.6: _s[0][0] /= 1.1
        while _s[0][0] < -1/1.6: _s[0][0] /= 1.1
        while _s[0][1] > 1/1.6: _s[0][1] /= 1.1
        while _s[0][1] < -1/1.6: _s[0][1] /= 1.1
...
while running:
    ...
    if m1:
        ...
        fill(pos1, abs(int(r1 / 1.1)), abs(int(r1 / 1.6)), c5, c6)
       #pygame.draw.circle(s, c4, pos1, abs(int(r1)))
        ...
        draw_shots(pos1, r1)
    fill(pos2, int(r2 * 1.05), int(r2), filler1, filler2)
    fill(pos2, int(r2), int(r2 * 3 / 4), c1, c2)
    #fill(pos2, int(r2 * 3 / 4), int(r2 / 2.5), c2, c3)
